...
option_list = list(
  make_option(c("-t", "--tables"), type="character", default="n", 
              help="Options are: [y/n]
              (default 'n')
              ATTENTION: --tables set to 'y' requires lots of computational power", metavar=""),
...
if(produce_tables){
  ...
  for(tumor_type in tumor_types){
    ...
    snv_filt <- snv[snv$patient_id %in% levels(factor(scna$patient_id)), ] 
    # keep patients with both snv and scna data
    scna <- scna[scna$patient_id %in% snv$patient_id, ] 
    common_patients <- levels(as.factor(snv[snv$patient_id %in% scna$patient_id, ]$patient_id))
    scna <- scna[scna$Segment_Mean >= 0.2 | scna$Segment_Mean <= -0.2, ]
    cat("  done!\n\n")
    cat("> common patients:", length(common_patients), "\n")
    
    proteins <- read.table("data/misc/protein-coding_gene.txt", header = FALSE)
    # downloaded from: ftp://ftp.ncbi.nlm.nih.gov/pub/CCDS/
    coding_regions <- read.table("data/misc/CCDS.current_hg19.txt", header = TRUE, sep = "\t")  
    # filter just protein coding genes
    coding_regions <- coding_regions[coding_regions$gene %in% proteins$V1,]                         
    coding_regions <- coding_regions[coding_regions$ccds_status == "Public",]
    coding_regions$length <- as.numeric(coding_regions$cds_to)-as.numeric(coding_regions$cds_from)
    coding_regions <- coding_regions[order(coding_regions$length, decreasing = T),]
    coding_regions <- coding_regions[!duplicated(coding_regions$gene),]
    total_pts <- length(levels(factor(scna$patient_id)))

    cat("> Load TPM File \n")
    tpm <- readRDS(file = paste0("data/TCGA_tpm/", tumor_type, "_tpm.rds.gz"))
    cat("  done!\n\n")
    ...
    results <- foreach(chr = 1:22, .combine = "rbind") %dopar% {
      ...
      for(i in 1:nrow(temp_coding)){
        ...
        cna_gene <- temp_cna[as.numeric(temp_cna$Start) <= as.numeric(temp_coding[i,]$cds_from) & as.numeric(temp_cna$End) >= as.numeric(temp_coding[i,]$cds_to),]
        # assess the frequency of the amplification
        amplified_pts <- levels(factor(cna_gene$patient_id))
        CN_pts <- common_patients[!common_patients %in% amplified_pts]
        
        gene_tpm <- tpm[tpm$patient_id %in% CN_pts & tpm$`Approved symbol` == gene_name,]
        gene_tpm <- mean(gene_tpm$tpm_counts, na.rm = T)
        
        freq_ampl <- length(levels(factor(cna_gene$patient_id)))/as.numeric(total_pts)
        
        n_mut_all <- dim(temp_mut[as.numeric(temp_mut$Start_Position) >= as.numeric(temp_coding[i,]$cds_from) &
                                    as.numeric(temp_mut$Start_Position) <= as.numeric(temp_coding[i,]$cds_to),])[1]/total_pts
        gene_length <- as.numeric(temp_coding$cds_to[i])-as.numeric(temp_coding$cds_from[i])
        n_mut_norm <- n_mut_all/gene_length
        
        n_pt_CN <- total_pts - length(amplified_pts)
        temp_mut_CN <- temp_mut[!temp_mut$patient_id %in% amplified_pts,]
        mutations_CN <- temp_mut_CN[as.numeric(temp_mut_CN$Start_Position) >= as.numeric(temp_coding[i,]$cds_from) &
        ...
        result <- rbind(result, cbind(Chromosome = chr,
                                      Gene = gene_name,
    ...
    # produce the tables
    write.csv(results, file = paste0(results_table_path,tumor_type,"_geneLevel_TRY.csv"))
  }
}
...
if(produce_statistics){
  ...
  for(tumor_type in tumor_types){
    results <- read.csv(paste0(results_table_path,tumor_type,"_geneLevel.csv"))
    
    results <- results[results$MutationsDiploid_norm != 0,]
    results <- results[results$Amplification != 0,]
    results <- results[!duplicated(paste0(results$Gene, results$GeneLegth)),]
    ...
    statistics <- rbind(statistics, cbind(tumor_type, 
                                          type = "gene",
    ...
    p1 <- ggplot(results, aes(y = y, x = x)) +
      ...
      ggtitle(paste0(tumor_type, " - All genes (",dim(results)[1],")"), subtitle = paste("Pearson's R =", signif(corP$estimate, digits = 3), "\n", "p-value =", signif(corP$p.value, digits = 3),
  ...
  results_table_statistics_path <- "results/tables/02_produceStatistics/"
...
cat("\t (3) gene-level correlations path:", results_table_statistics_path, "\n\n")