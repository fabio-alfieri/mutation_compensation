  ...
  for (segment_length in segment_lengths) {
    ...
    for (condition in conditions) {
      if (condition == "amplifications" | condition == "deletions") {
        source_table_path <-
          "results/tables/01_binLevelAnalysis/all_mutations_vs_CN/"
      } else{
        source_table_path <-
      ...
      suppressMessages({
        for (tumor_type in tumor_types) {
          ...
          if (chr_arm == "yes") {
            ampl_freq <-
              read.table(file = paste0(source_table_path, tumor_type, "_chrAmpFreq.txt"))
    ...
    if (chr_arm == "yes") {
      ...
      write.table(
        parameters_chr,
        file = paste0(
          "results/tables/02_produceStatistics/02d_statistics_chromosome.txt"
        ),
        quote = F,
        row.names = F
      )
      write.table(
        parameters_arm,
        file = paste0(
          "results/tables/02_produceStatistics/02c_statistics_arm.txt"
        ),
        quote = F,
        row.names = F
      )
...
if (produce_statistics) {
  ...
  for (tumor_type in tumor_types) {
    ...
    scna_ampl <-
      scna[scna$Segment_Mean >= 0.2 & scna$length >= 500000, ]
    scna_del <-
      scna[scna$Segment_Mean <= -0.2 & scna$length >= 500000, ]
    length_total_ampl <-
  ...
  pdf(
    "results/plots/000_paper_plots/00_Fig1b.pdf",
    width = 14,
    height = 8
  )
  print(p1)
