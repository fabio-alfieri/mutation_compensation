...
for(tumor_type in tumor_types){
  ...
  tuson_OGs <- read_xlsx("data/misc/Davoli2013_TUSON.xlsx", sheet = 2)  
  tuson_TSGs <- read_xlsx("data/misc/Davoli2013_TUSON.xlsx", sheet = 1)
  ...
  if (tumor_type == "BRCA") {
    ...
    genes_in_bin$TSG.status <-
      ifelse(genes_in_bin$gene %in% tuson_TSGs[order(tuson_TSGs$`Breast_p-value`, decreasing = F), ]$Gene[1:nTSGs], "TSG", "noTSG")
    ...
    genes_in_bin$TSG.status <-
      ifelse(genes_in_bin$gene %in% tuson_TSGs[order(tuson_TSGs$`Pancreas_p-value`, decreasing = F), ]$Gene[1:nTSGs], "TSG", "noTSG")
  ...
  } else if (tumor_type == "PAAD") 
  {
    ...
    library1$GO.status <-
      ifelse(library1$HPNE.FDR <= 0.01 &
               library1$HPNE.Log2 > 1, "GO", "noGO")
    colnames(library1)[2] <- "gene"
    library2$GO.status <-
      ifelse(library2$HPNE.FDR <= 0.01 &
               library2$HPNE.Log2 > 1, "GO", "noGO")
    colnames(library2)[2] <- "gene"
  ...
  merged <- full_join(bin_values, GO.scores, by = "bin_id_merged36")
  ...
  if (CTR == T) {
    ...
    cat("Amplification frequency of",
        tumor_type,
        "is replace with",
        tumor_type_control)
    bin_values <-
  ...
  # SPEARMAN ----
  cor.test(merged$cna_freq_ampl, as.numeric(merged$og.score), method = "spearman")
  cor.test(merged$cna_freq_ampl, as.numeric(merged$go.score), method = "spearman")
  mu_score <- cor.test(merged$cna_freq_ampl, log10(as.numeric(merged$mutations_norm)), method = "spearman")
  ...
  a1 <- ggscatter(
    ...
    ggtitle(label = tumor_type, subtitle = paste("ampl.f ~ OG.score"))
  ...
  a2 <- ggscatter(
    ...
    ggtitle(label = tumor_type, subtitle = paste("ampl.f ~ GO.score"))
  ...
  a4 <- ggscatter(
    ...
    ggtitle(label = tumor_type, subtitle = paste("ampl.f ~ MUT.score"))
  ...
  pdf(
    file = paste0("results/plots/00_paper_plots/00_SupplementaryFig3_", tumor_type, ".pdf"),
    width = 12,
    height = 7
  )
...
p1 <- ggplot(stat_estimates, aes(x = as.factor(tumor_types), y = rho)) +
  geom_bar(stat = "identity", position = "dodge", aes(fill = as.factor(condition))) +
  theme_classic() + scale_fill_brewer(palette="Blues")

p2 <- ggplot(stat_signif, aes(x = as.factor(tumor_types), y = log10(p.val))) +
  geom_bar(stat = "identity", position = "dodge", aes(fill = as.factor(condition))) +
  theme_classic() + scale_fill_brewer(palette="Reds")

pdf("results/plots/00_paper_plots/00_Fig3.pdf", height = 10)
print(ggarrange(p1, p2, ncol = 1))
dev.off()


if(CTR == T){
  colnames(stat_estimates_CTR) <- c("tumor_types", "rho", "condition")
  stat_estimates_CTR$rho <- as.numeric(stat_estimates_CTR$rho)
  
  colnames(stat_signif_CTR) <- c("tumor_types", "p.val", "condition")
  stat_signif_CTR$p.val <- as.numeric(stat_signif_CTR$p.val)
  
  p1 <- ggplot(stat_estimates_CTR, aes(x = as.factor(tumor_types), y = rho)) +
    geom_bar(stat = "identity", position = "dodge", aes(fill = as.factor(condition))) +
    theme_classic() + scale_fill_brewer(palette="Blues")
  
  p2 <- ggplot(stat_signif_CTR, aes(x = as.factor(tumor_types), y = log10(p.val))) +
    geom_bar(stat = "identity", position = "dodge", aes(fill = as.factor(condition))) +
    theme_classic() + scale_fill_brewer(palette="Reds")
  
  pdf("results/plots/00_paper_plots/00_SupplementaryFig4.pdf", height = 10)
  print(ggarrange(p1, p2, ncol = 1))
  dev.off()
}
