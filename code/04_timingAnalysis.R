...
if(run_CNAqc_analysis & !test){
  ...
  for(pcawg_id in patientsPCAWG$pt){
    ...
    try({
      ...
      CCF_table$purity <- patientsPCAWG[patientsPCAWG$pt == pcawg_id,]$purity
      CCF_table$ploidy <- patientsPCAWG[patientsPCAWG$pt == pcawg_id,]$ploidy
      CCF_table$tumor_type <- patientsPCAWG[patientsPCAWG$pt == pcawg_id,]$tumor_type
      
      write_rds(CCF_table, paste0("results/tables/04_timingAnalysis/tmp/",pcawg_id,"_CCF.rds"))
    })
  }
  ...
  for(file in paste0(patientsPCAWG$pt, "_CCF.rds")){
    try({
      pt_file <- readRDS(paste0("results/tables/04_timingAnalysis/tmp/",file))
...
mutations_wCCF$multiplicity <- as.numeric(as.character(mutations_wCCF$multiplicity))

intergenic_region <- mutations_wCCF %>% filter(consequence_type == "intergenic_region")
intergenic_region_mixed <- mutations_wCCF %>% filter(consequence_type == "intergenic_region:upstream_gene_variant" |
...
exon_variant  <- mutations_wCCF %>% filter(consequence_type == "exon_variant")
exon_variant_mixed  <- mutations_wCCF %>% filter(consequence_type == "intron_variant:exon_variant" |
...
missense_variant <- mutations_wCCF %>% filter(consequence_type == "missense_variant")
missense_variant_mixed <- mutations_wCCF %>% filter(consequence_type == "missense_variant:exon_variant"|
...
synonymous_variant <- mutations_wCCF %>% filter(consequence_type == "synonymous_variant")

# define coding and non-coding mutations
non_coding <- rbind(intergenic_region, intergenic_region_mixed)
coding <- rbind(missense_variant, missense_variant_mixed, 
                exon_variant,exon_variant_mixed, 
                synonymous_variant)
...
timing_kar <- function(ta){
  ...
  ta <- rbind(ta2.0, ta2.1, ta2.2, ta3.0, ta3.1, ta3.2, ta3.3, ta4.0, ta4.1, ta4.2, ta4.3, ta5.0, ta5.1, ta5.2)
  return(ta)
}
...
for(kar in c("2:0", "2:1", "2:2", "3:0", "3:1", "3:2", "3:3", "4:0", "4:1", "4:2", "4:3", "5:0", "5:1", "5:2")){
  ...
  par(mfrow = c(1,2))
  print(ggplot(mutations_wCCF %>% filter(karyotype == kar)) +
          geom_histogram(aes(x = VAF/purity, fill = multiplicity), alpha = 0.3, binwidth = 0.01) +
          theme_bw() +
          xlim(0, 1.3) +
          ggtitle(paste("PCAWG - karyotype",kar)))
  pie(table(t$timing)/sum(table(t$timing)), main = paste(kar, "- coding mutations"))
  pie(table(p$timing)/sum(table(p$timing)), main = paste(kar, "- non-coding mutations"))
  
}
dev.off()

rbind(table(non_coding_timing$timing),table(coding_timing$timing))[,1:2]
fisher.test(rbind(table(non_coding_timing$timing),table(coding_timing$timing))[,1:2], alternative = "greater")
chisq.test(rbind(table(non_coding_timing$timing),table(coding_timing$timing))[,1:2])
...
test_independence <- function(cod, ncod, kar){
  ...
  t_perc <- rbind(table(ncod$timing)/sum(table(ncod$timing)), table(cod$timing)/sum(table(cod$timing)))
  
  print(t_perc)
  n_mut <- dim(cod)[1] + dim(ncod)[1]
  return(
    append(fisher.test(t, alternative = "greater"), Barnard(t), n_mut)
  )
}
...
for(kar in c("2:0", "2:1", "2:2", "3:0", "3:1", "3:2", "3:3", "4:0", "4:1", "4:2", "4:3", "5:0", "5:1", "5:2")){
  ...
  ftot <- rbind(ftot, cbind(karyotype = kar, odds.ratio = f$estimate, p.value = f$p.value, n_muts = f[[8]]))
}
...
timed_mutations <- as.data.frame(table(timed_mutations$karyotypeF, timed_mutations$timing))
